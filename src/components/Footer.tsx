      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="grid gap-8 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="text-sm text-secondary-foreground/60 font-sans">{contact.address}</p>
            {contact.phones.map((p) => (
              <a key={p} href={`tel:${p}`} className="block text-sm text-secondary-foreground/60 font-sans hover:text-primary">
                {p}
              </a>
            ))}
          </div>
        ...
        <div className="mt-10 border-t border-white/10 pt-6 text-center">
          <p className="text-xs text-secondary-foreground/40 font-sans">{footer.copyright}</p>
        </div>
      </div>
    </footer>
  );
